...
# соответствует сдвигу вправо. На второй строке указывается непустая фраза для шифрования. Ведущие и завершающие
...
def cipher(shift, string, alfablet):
    result = ''
    i = 0
    while i <= len(string) - 1:
        s = string[i]
        a = alfablet.index(s)
        aa = (a + shift) % len(alfablet)
        result += alfablet[aa]
        i += 1
    print('Result:' + ' "' + '{}'.format(result) + '"')


alfablet = ' abcdefghijklmnopqrstuvwxyz'
shift = int(input())
string = input().strip()
cipher(shift, string, alfablet)
